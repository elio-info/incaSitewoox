let equip_INCA_proy= [
    { 
        cargo:" * Investigador Titular",
        name:" Michel Ruiz S&aacute;nchez",
        titulo:"Dr.C. Agr&oacute;nomas Ingeniero Agr&oacute;nomo",
        pict_cargo:"user-tie",
        lugar:"UCTB Los Palacios. Dpendencia Instituto Nacional de Ciencias Agr&iacute;colas (INCA).",
        pict_tipo_personal:"tree",
        dato :"Personal Directivo del Proyecto",
        cv:"Michel"
    }
    ,
    { 
        cargo:" Investigador ",
        name:" Alexei Yoan Mart&iacute;nez Robaina ",
        titulo:"Dr.C. Agroecol&oacute;gicas.  Ingeniero Agr&oacute;nomo",
        pict_cargo:"user",
        lugar:"Universidad de Pinar del R&iacute;o (UPR) 'Hermanos Sa&iacute;z Montes de Oca'" ,
        pict_tipo_personal:"tree",
        dato :"Personal del Proyecto",
        cv:"Alexei"
    }
    ,
    { 
        cargo:" Investigador ",
        name:" Edel jes&uacute;s Amaro Aroche ",
        titulo:"M.Sc. Agroecol&oacute;gicas.  Ingeniero Agr&oacute;nomo",
        pict_cargo:"user",
        lugar:"Instituto de Suelos de Cuba",
        pict_tipo_personal:"tree",
        dato :"Personal del Proyecto",
        cv:"Edel"
    }
    ,
    { 
        cargo:" Investigador ",
        name:" Edenysl Miranda Izquierdo ",
        titulo:"M.Sc. Agroecol&oacute;gicas.  Ingeniero Agr&oacute;nomo",
        pict_cargo:"user",
        lugar:"(UPR) 'Hnos Sa&iacute;z Montes de Oca' ",
        pict_tipo_personal:"tree",
        dato :"Personal del Proyecto",
        cv:"Edenys"
    }
    ,
    { 
        cargo:" Inform&aacute;tico ",
        name:" Eliomar Rodr&iacute;guez Izquierdo ",
        titulo:"M.Sc. Tecnolog&iacute;s inform&aacute;cas.  Ingeniero Inform&aacute;tico",
        pict_cargo:"user",
        lugar:"Universidad de Pinar del R&iacute;o (UPR) 'Hermanos Sa&iacute;z Montes de Oca'",
        pict_tipo_personal:"tree",
        dato :"Inform&aacute;tico del Proyecto",
        cv:"Eliomar"
    }
    ,
    { 
        cargo:" Investigador ",
        name:" Yaumara Muñoz Hern&aacute;ndez ",
        titulo:"M.Sc. Tecnolog&iacute;as en la educaci&oacute;n.  licenciado en Educaci&oacute;n",
        pict_cargo:"user",
        lugar:"Universidad de Pinar del R&iacute;o (UPR) 'Hermanos Sa&iacute;z Montes de Oca'",
        pict_tipo_personal:"tree",
        dato :"Personal del Proyecto",
        cv:"Yaumara"
    }
    ,
    { 
        cargo:" Investigador ",
        name:" Ram&oacute;n Antonio Rivera Espinosa ",
        titulo:"Dr.C. Agr&iacute;colas.  Licenciado Qu&iacute;mica",
        pict_cargo:"user",
        lugar:"Instituto Nacional de Ciencias Agr&iacute;colas (INCA) ",
        pict_tipo_personal:"tree",
        dato :"Personal del Proyecto",
        cv:"Ramon"
    }
    ,
    { 
        cargo:" Investigador ",
        name:" Yoerlandy Santana Baños ",
        titulo:"M.Sc. Agroecol&oacute;gicas.  Ingeniero Agr&oacute;nomo",
        pict_cargo:"user",
        lugar:"Universidad de Pinar del R&iacute;o (UPR) 'Hermanos Sa&iacute;z Montes de Oca'",
        pict_tipo_personal:"tree",
        dato :"Personal del Proyecto",
        cv:"Santana"
    }
    ,
    { 
        cargo:" Investigador ",
        name:" Yanara Regla Huele Arriete ",
        titulo:" Ingeniero Agr&oacute;nomo",
        pict_cargo:"user",
        lugar:"Instituto Nacional de Suelos de Cuba ",
        pict_tipo_personal:"tree",
        dato :"Personal del Proyecto",
        cv:"Yanara"
    }
]
